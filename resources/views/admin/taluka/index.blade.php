...
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">Taluka List</h4>
    ...
    <div class="col-12">
        <div class="card p-1">
            <table id="taluka" class="datatables-basic table table-hover">
                <thead>
                    <tr>
                        <th>#</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-transparent border-bottom">
                <h4 class="text-center mb-0" id="exampleModalTitle">Add Taluka</h4>
                ...
                <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            ...
                            <select class="form-control form-select select2 custom-select2" name="district_id" id="district_id">
                                <option selected disabled>{{ __('message.-- Select --') }}</option>
                                @foreach($district as $value)
                                <option value="{{$value->id}}">{{$value->name}} {{ (!is_null($value->state)) ? ' - '. $value->state->state_name : '' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
...
<script type="application/javascript">
    'use strict';
    const URL = "{{route('taluka.index')}}";

    var table = '';
    $(function() {
        table = $('#taluka').DataTable({
            ajax: URL,
            processing: true,
            serverSide: true,
            fixedHeader: true,
    ...
    $("#inlineModal").on("hidden.bs.modal", function(e) {
        ...
        $("#exampleModalTitle").html("Add Taluka");
    });

    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        var url = "{{route('taluka.edit','id')}}".replace('id', id);
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
            ...
            success: function(data) {
                if (data.msg_type == "success") {
                    $("#exampleModalTitle").html("Edit Taluka");
                    ...
                    axios.delete(URL + '/' + id)
                        .then(function(response) {
                            if (response.data.status == true) {
                                ...
                                toastr.warning(response.data.message, "{{ __('message.Warning') }}");
                            }
                        })
                        .catch(function() {
                            toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
