@extends('layouts.app')
@section('title', 'Agent Sales Person')
@section('content')
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">Agent Sales Person List</h4>

        @can('agent-sales-create')
    ...
    <div class="col-12">
        <div class="card p-1">
            <table id="agent_sales_person" class="datatables-basic table table-hover">
                <thead>
                    <tr>
                        <th>#</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-transparent border-bottom">
                <h4 class="text-center mb-0" id="exampleModalTitle">Add Agent Sales Person</h4>
                ...
                <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            <input type="hidden" name="agent_sales_person_id" id="agent_sales_person_id" value="">
                        ...
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            ...
                            <input type="number" maxlength="10" class="form-control" name="number" id="number" placeholder="Number">
...
<script type="application/javascript">
    ...
    'use strict';
    const URL = "{{route('agent-sales-person.index')}}";

    var table = '';
    $(function() {
        table = $('#agent_sales_person').DataTable({
            ajax: URL,
            processing: true,
            serverSide: true,
            fixedHeader: true,
    ...
    $(document).on('click', '.save', function() {
        ...
        if ($("#form").valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "{{route('agent-sales-person.store')}}",
                data: formData,
                dataType: 'json',
                cache: false,
                contentType: false,
    ...
    $("#inlineModal").on("hidden.bs.modal", function(e) {
        ...
        $("#name-error").html("");
        $("#number-error").html("");
        $("#exampleModalTitle").html("Add Agent Sales Person");
    });

    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        var url = "{{route('agent-sales-person.edit','id')}}".replace('id', id);
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
            ...
            success: function(data) {
                if (data.msg_type == "success") {
                    $("#exampleModalTitle").html("Edit Agent Sales Person");
                    $("#name").val(data.result.name);
                    $("#number").val(data.result.number);
                    $("#agent_sales_person_id").val(id);
                    ...
                    axios.delete(URL + '/' + id)
                        .then(function(response) {
                            if (response.data.status == true) {
                                ...
                                toastr.warning("{{ __('message.This agent-sales-person has been used.') }}", "{{ __('message.Warning') }}");
                            }
                        })
                        .catch(function() {
                            toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
