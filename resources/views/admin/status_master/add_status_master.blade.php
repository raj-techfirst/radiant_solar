     ...
     <div class="col-12">
          <div class="card">
               <div class="card-header">
                    ...
                    <h3 class="card-title">{{ __('message.Edit') }} <small>{{ __('message.Status') }}</small></h3>
                    @else
                    <h3 class="card-title">{{ __('message.Add') }} <small>{{ __('message.Status') }}</small></h3>
                    @endif
               </div>
               <div class="card-body">
                    <form id="form" class="form" action="javascript:void(0);" method="POST">
                         ...
                         <div class="row">
                              ...
                              <div class="col-12 col-md-12 mb-1 custom-input-group">
                                   <label class="form-label" for="status_name">{{ __('message.Status') }}<span class="text-danger">*</span></label>
                                   <input type="text" class="form-control" name="status_name" id="status_name" placeholder="{{ __('message.Status Name') }}" value="{{ ((isset($statusMaster) && isset($statusMaster->status_name)) ? $statusMaster->status_name : old('status_name'))  }}">
                                   <span class="invalid-feedback d-block" id="error_status_name" role="alert"></span>
                              </div>
                              <div class="col-md-12 col-12 custom-input-group">
                                   <button type="submit" class="btn btn-primary float-end save">{{ __('message.Submit') }}</button>
                              </div>
                         </div>
                    </form>
               </div>
     ...
     $(document).on('click', '.save', function() {
          ...
          if ($("#status_name").val() != "") {
               ...
               $("#form").validate({
                    ...
                    messages: {
                         status_name: {
                              required: "{{ __('message.Enter status') }}"
                         },
                    },
                    errorElement: "p",
