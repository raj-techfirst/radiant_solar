...
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">Village List</h4>
    ...
    <div class="col-12">
        <div class="card p-1">
            <table id="village" class="datatables-basic table table-hover">
                <thead>
                    <tr>
                        <th>#</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-transparent border-bottom">
                <h4 class="text-center mb-0" id="exampleModalTitle">Add Village</h4>
                ...
                <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            ...
                            <select class="form-control form-select select2 custom-select2 district_id" name="district_id" id="district_id">
                        ...
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            ...
                            <select class="form-control form-select select2 custom-select2 taluka_id" name="taluka_id" id="taluka_id">
                                <option selected disabled>{{ __('message.-- Select --') }}</option>
                            </select>
                        </div>
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            <label class="form-label" for="name">Village Name</label>
...
<script type="application/javascript">
    ...
    $("#inlineModal").on("hidden.bs.modal", function(e) {
        ...
        $("#taluka_id").val("");
        $("#name-error").html("");
        $("#district_id-error").html("");
        $("#taluka_id-error").html("");
        $("#exampleModalTitle").html("Add Village");
    });

    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        var url = "{{route('village.edit','id')}}".replace('id', id);
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
            ...
            success: function(data) {
                if (data.msg_type == "success") {
                    $("#exampleModalTitle").html("Edit Village");
                    $("#name").val(data.result.name);
                    $("#district_id").val(data.result.district_id).trigger('change');
                    $("#taluka_id").val(data.result.taluka_id).trigger('change');
                    $("#village_id").val(id);
                    // $("#district_id").val(id);
                    ...
                    axios.delete(URL + '/' + id)
                        .then(function(response) {
                            if (response.data.status == true) {
                                ...
                                toastr.warning("{{ __('message.This Village has been used.') }}", "{{ __('message.Warning') }}");
                            }
                        })
                        .catch(function() {
                            toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
