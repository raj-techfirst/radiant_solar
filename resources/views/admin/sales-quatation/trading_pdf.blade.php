...
<body>
        ...
        <tr>
            ...
            <td class="text_right" style="padding-left: 5px;width: 50%;">
                <b>{{ $sales_data->name }}<br>
                    ...
                    @if ($sales_data->gst_no != '')
                        <br>GSTIN: {{ $sales_data->gst_no }}
                    @endif
                </b>
            </td>
        </tr>
    ...
    <table border="1">
            ...
            <tr>
                ...
                @if ($value->type == 'Item')
                    <td class="text-nowrap">{{ $value->item->name }}
                        @if ($value->item->hsn_code != '')
                            <br>HSN/SAC Code: {{ $value->item->hsn_code }}
                        @endif
                    </td>
                @else
                    <td class="text-nowrap">{{ getItemGropName($value, 1) }}
                        @if ($value->itemGroup->hsn_code != '')
                            <br>HSN/SAC Code: {{ $value->itemGroup->hsn_code }}
                        @endif
                    </td>
                @endif
                ...
                <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span>
                    {{ number_format($value->rate, 2) }}</td>
                <td class="text_right">{{ $sales_data->gst == 'Extra' ? 'GST ' . $value->item_gst . '%' : 'GST 0%' }}
                </td>
                <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span>
                    {{ number_format($value->nos * $value->rate, 2) }}</td>
            </tr>
            @php
        ...
        <tr>
            ...
            <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span>
                {{ number_format($finalsub, 2) }}</td>
        </tr>
        <tr>
            <td colspan="3"></td>
            @if ($sales_data->gst == 'Extra')
                <td colspan="2" class="text_right">SGST on <span
                        style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format($finalsub, 2) }}
                </td>
            @else
                <td colspan="2" class="text_right">SGST</td>
            @endif
            <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span>
                {{ $sales_data->gst == 'Extra' ? number_format($finalsubTota / 2, 2) : '0.00' }}</td>
        </tr>
        <tr>
            <td colspan="3"></td>
            @if ($sales_data->gst == 'Extra')
                <td colspan="2" class="text_right">CGST on <span
                        style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format($finalsub, 2) }}
                </td>
            @else
        ...
        <tr>
            ...
            <td class="text_right"><b><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span>
    ...
    {!! isset($policy_data) && $policy_data ? $policy_data->policy : '' !!}
</body>
