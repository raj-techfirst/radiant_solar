...
<body>
     <h2 style="font-size:22px;">SOLAR POWER PLANT PROPOSAL FOR {{ strtoupper($sales_data->quatation_type) }} PROJECT</h2>
     <hr>
     <br />
     <table>
          <tr>
               <td>Ref : {{ env('APP_SORT') }}/EPC/{{str_pad($sales_data->id, 2, '0', STR_PAD_LEFT)}}</td>
               <td style="text-align: right;"><b>Date : {{date('d-m-Y', strtotime($sales_data->created_at))}} </b></td>
          <tr>
     ...
     <br />
     <p>We Look Forward To Hear From You Shortly.</p>
     <p>Thanking You,</p>
     ...
     <div class="page-break"></div>
     <h3 style="text-align: center;">Commercial Offer</h3>
     <table border="1">
          <tr>
          ...
          <tr>
               ...
               <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format($calc['val4'],2) }}</td>
               <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format($calc['rate_per_kw_value'],2) }}</td>
          </tr>
          <tr>
               <td>2</td>
               <td><b>Payable GST Amount :- </b><br>On 70% of Contract Value as Goods, Taxable @ 5% On Balance 30% as services, Taxable @ 18%</td>
               <td></td>
               <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format($calc['rate_per_kw_gst'],2) }}</td>
               <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format($calc['rate_per_kw_value_gst'],2) }}</td>
          </tr>
          <tr>
               <td colspan="4" class="text_right"><b>Total Project With GST Cost</b></td>
               <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format(($calc['rate_per_kw_value'] + $calc['rate_per_kw_value_gst']),2) }}</td>
          </tr>
          <tr>
               <td colspan="4" class="text_right">DISCOM Meter Connectivity Charge (Approx)</td>
               <td class="text_right">
          ...
          <tr>
               ...
               <td class="text_right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{number_format($calc['rate_per_kw_value'] + $calc['rate_per_kw_value_gst'] + $sales_data->meter_charges +$sales_data->registration_fee + $sales_data->other_charge_amount,2)}}</td>
          </tr>
     </table>

     <h3 style="text-align: center;">Technical Specifications & BOM</h3>
     <table border="1">
          <tr>
               <th>Sr. no.</th>
     ...
     <p><b>Name: -</b> {{$sales_data->holder_name}}<br /> <b>Bank Name : -</b> {{$sales_data->bank_name}}<br />
          ...
          <b>Bank Branch: -</b> {{$sales_data->branch}}
     </p>
</body>
