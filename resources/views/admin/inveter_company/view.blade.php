@extends('layouts.app')
@section('title', 'Panel Company')
@section('content')
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">Panel Company List</h4>
       {{-- @can('category-creat')
        <a role="button" class="btn btn-sm btn-primary float-end" href="{{route('penal-company.create')}}"><i class="fa fa-plus me-25"></i> {{ __('message.Add New') }}</a>
        @endcan--}} 
       {{--@can('category-create')--}} 
    ...
    <div class="col-12">
        <div class="card p-1">
            <table id="penal_company" class="datatables-basic table table-hover">
                <thead>
                    <tr>
                        <th>#</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-transparent border-bottom">
                <h4 class="text-center mb-0" id="exampleModalTitle">Add Panel Company</h4>
                ...
                <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            <input type="hidden" name="penal_company_id" id="penal_company_id" value="">
...
<script type="application/javascript">
    'use strict';
    const URL = "{{route('penal-company.index')}}";

    var table = '';
    $(function() {
        table = $('#penal_company').DataTable({
            ajax: URL,
            processing: true,
            serverSide: true,
            fixedHeader: true,
    ...
    $("#inlineModal").on("hidden.bs.modal", function(e) {
        ...
        $("#penal_company_id").val("");
        $("#name-error").html("");
        $("#exampleModalTitle").html("Add Panel Company");
    });

    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        var url = "{{route('penal-company.edit','id')}}".replace('id', id);
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
            ...
            success: function(data) {
                if (data.msg_type == "success") {
                    $("#exampleModalTitle").html("Edit Panel Company");
                    $("#name").val(data.result.name);
                    $("#penal_company_id").val(id);
                    ...
                    axios.delete(URL + '/' + id)
                        .then(function(response) {
                            if (response.data.status == true) {
                                ...
                                toastr.warning("{{ __('message.This Panel Company has been used.') }}", "{{ __('message.Warning') }}");
                            }
                        })
                        .catch(function() {
                            toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
