@extends('layouts.app')
@section('title', 'Inverter Company')
@section('content')
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">Inverter Company List</h4>
       
       @can('inveter-company-create') 
    ...
    <div class="col-12">
        <div class="card p-1">
            <table id="inveter_company" class="datatables-basic table table-hover">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>{{ __('message.Action') }}</th>
                        <th>Inverter Company Name</th>
                    </tr>
                </thead>
                <tbody>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-transparent border-bottom">
                <h4 class="text-center mb-0" id="exampleModalTitle">Add Inverter Company</h4>
                ...
                <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            <input type="hidden" name="inveter_company_id" id="inveter_company_id" value="">
...
<script type="application/javascript">
    'use strict';
    const URL = "{{route('inveter-company.index')}}";

    var table = '';
    $(function() {
        table = $('#inveter_company').DataTable({
            ajax: URL,
            processing: true,
            serverSide: true,
            fixedHeader: true,
    ...
    $(document).on('click', '.save', function() {
        ...
        if ($("#name").val() != "") {
            $.ajax({
                type: "POST",
                url: "{{route('inveter-company.store')}}",
                data: formData,
                dataType: 'json',
                cache: false,
                contentType: false,
    ...
    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        var url = "{{route('inveter-company.edit','id')}}".replace('id', id);
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
