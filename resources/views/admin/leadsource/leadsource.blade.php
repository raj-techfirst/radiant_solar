...
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">{{ __('message.Lead Source List') }}</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-transparent border-bottom">
                <h4 class="text-center mb-0" id="exampleModalTitle">{{ __('message.Add Lead Source') }}</h4>
                ...
                <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12 mb-1 custom-input-group">
                            ...
                            <input type="text" class="form-control" name="source_name" id="source_name" placeholder="{{ __('message.Lead Source') }} *">
...
<script type="application/javascript">
    'use strict';
    const URL = "{{route('lead-source.index')}}";
    ...
    $(document).on('click', '.save', function() {
        ...
        if ($("#source_name").val() != "") {
            ...
            $("#form").validate({
                ...
                messages: {
                    source_name: {
                        required: "{{ __('message.Enter Lead Source') }}"
                    },
                },
                errorElement: "p",
    ...
    $("#inlineModal").on("hidden.bs.modal", function(e) {
        ...
        $(".custom-error").html("");
        $("#exampleModalTitle").html("{{ __('message.Add Lead Source') }}");
    });

    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        var url = "{{route('lead-source.edit','id')}}".replace('id', id);
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
            ...
            success: function(data) {
                if (data.msg_type == "success") {
                    $("#exampleModalTitle").html("{{ __('message.Edit Lead Source') }}");
                    $("#source_name").val(data.result.source_name);
                    $("#source_id").val(id);
                    ...
                    axios.delete(URL + '/' + id)
                        .then(function(response) {
                            if (response.data.status == true) {
                                ...
                                toastr.warning("{{ __('message.This Lead Source has been used.') }}", "{{ __('message.Warning') }}");
                            }
                        })
                        .catch(function() {
                            toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
