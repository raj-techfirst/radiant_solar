...
<div class="row">
    <div class="col-12 mb-50">
        <h4 class="content-header-title float-start">Warehouse Stock Transfer</h4>
        @can('warehouse-stock-list')
        <a href="{{ route('warehouse-stock.index') }}" class="btn btn-sm btn-primary float-end"><i class="fa fa-angle-double-left me-25"></i> List</a>
        @endcan
    </div>
    <div class="col-12">
        <div class="card p-1">
            <form id="form" class="form" action="javascript:void(0);" method="POST">
                ...
                <div class="row">
                    <div class="col-12 col-md-6 col-lg-4 mb-1 custom-input-group">
                        ...
                        <label class="form-label" for="warehouse_out">Out <span class="text-danger">*</span></label>
                        <select class="form-select select2" name="warehouse_out" id="warehouse_out">
                    ...
                    <div class="col-12 col-md-6 col-lg-4 mb-1 custom-input-group">
                        <label class="form-label" for="warehouse_in">In <span class="text-danger">*</span></label>
                        <select class="form-select select2" name="warehouse_in" id="warehouse_in">
                            <option selected disabled>{{ __('message.-- Select --') }}</option>
                            @foreach($warehouse as $value)
                            <option value="{{ $value->id }}">{{ $value->name }}</option>
                            @endforeach
                    ...
                    <div class="col-12 my-25 d-none" id="table-col">
                        <table class="table table-sm table-bordered purchase-table w-auto">
                            <thead>
                                <tr>
...
<script type="application/javascript">
    $(document).on('change', '#warehouse_out', function() {
        var id = $(this).val();
        if (id != "") {
    ...
    $("#form").validate({
        ...
        messages: {
            warehouse_out: {
                required: "Select warehouse out",
            },
            warehouse_in: {
                required: "Select warehouse in",
            },
            "quantity[]": {
