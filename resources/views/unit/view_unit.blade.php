...
<div class="row">
    <div class="col-12 mb-1">
        <h4 class="content-header-title float-start">{{ __('message.View Unit') }}</h4>
        @can('unit-create')
        <a role="button" class="btn btn-sm btn-primary float-end" href="{{route('unit.create')}}"><i class="fa fa-plus me-25"></i> {{ __('message.Add New') }}</a>
        @endcan
    </div>
    <div class="col-12">
        <div class="card p-2">
            <table id="tabel" class="datatables-basic table table-hover">
                <thead>
                    <tr>
                        <th>#</th>
                    ...
                    axios.delete(URL + '/' + id)
                        .then(function(response) {
                            if (response.data.status == true) {
                                ...
                                toastr.warning("{{ __('message.This unit has been used.') }}", "{{ __('message.Warning') }}");
                            }
                        })
                        .catch(function() {
                            toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
