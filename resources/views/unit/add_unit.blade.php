...
<div class="row">
     <div class="col-12">
          @if((isset($unit) && isset($unit->id)))
          <h4 class="card-title mb-1">{{ __('message.Edit') }} <small>{{ __('message.Unit') }}</small></h4>
          @else
          <h4 class="card-title mb-1">{{ __('message.Add') }} <small>{{ __('message.Unit') }}</small></h4>
          @endif
     </div>
     <div class="col-12">
          <div class="card p-1">
               <form id="form" class="form" action="javascript:void(0);" method="POST">
                    ...
                    <div class="row">
                         ...
                         <div class="col-12 col-md-12 mb-1 custom-input-group">
                              <label class="form-label" for="unit_name">{{ __('message.Unit') }}<span class="text-danger">*</span></label>
                              <input type="text" class="form-control" name="unit_name" id="unit_name" placeholder="{{ __('message.Unit') }}" value="{{ ((isset($unit) && isset($unit->unit_name)) ? $unit->unit_name : old('unit_name'))  }}">
                              <span class="invalid-feedback d-block" id="error_unit_name" role="alert"></span>
                         </div>
                         <div class="col-md-12">
     ...
     $(document).on('click', '.save', function() {
          ...
          if ($("#unit_name").val() != "") {
               ...
               $("#form").validate({
                    ...
                    messages: {
                         unit_name: {
                              required: "{{ __('message.Enter unit') }}"
                         },
                    },
                    errorElement: "p",
