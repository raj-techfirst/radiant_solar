...
<div class="row">
     <div class="col-12 mb-1">
          <h4 class="content-header-title float-start">{{ __('message.Sales Officer List') }}</h4>
          @can('sales-create')
          <a role="button" class="btn btn-sm btn-primary float-end" href="{{route('sales.create')}}"><i class="fa fa-plus me-25"></i> {{ __('message.Add New') }}</a>
          @endcan
     </div>
     <div class="col-12">
          <div class="card p-1">
...
<script type="text/javascript">
     ...
     $(document).on('click', '.status', function() {
          ...
          let id = $(this).data('id');
          var route = "{{route('sales.show','id')}}".replace('id', id);
          $.ajax({
               type: "get",
               url: route,
               dataType: 'json',
                         ...
                         axios.delete(URL + '/' + id)
                              .then(function(response) {
                                   if (response.data.status == true) {
                                        ...
                                        toastr.warning("{{ __('message.This sales officer activity has been used.') }}", "{{ __('message.Warning') }}");
                                   }
                              })
                              .catch(function() {
                                   toastr.error("{{ __('message.Something went wrong. Please try again.') }}", "{{ __('message.Error') }}");
