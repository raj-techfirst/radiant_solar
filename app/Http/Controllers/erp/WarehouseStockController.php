...
class WarehouseStockController extends Controller
{
            ...
            if (!empty($request->type) && $request->type == 'Adjust') {
                ...
                if ($request->project_ids != '') {
                    ...
                    foreach ($salesOrder as $bomId => $bomCount) {
                        $bomDataLocal = BOMMeta::select(
                            'type',
                            'item_id',
                            'item_group_id',
                            DB::raw('SUM(quantity) * ' . $bomCount . ' as quantity'),
                            'unit_id'
  ...
  $lastPrice = getLastPrice($item_id,'Item');
                            if ($request->quotations_id != '') {
                                $lastPrice = SalesQuatationMeta::selectRaw('rate')->where('item_id', $item_id)->where('sales_quatation_id', $request->quotations_id)->first()->rate ?? 0;
                            }
...
$lastPrice = getLastPrice($item_id,'ItemGroup');
                            if ($request->quotations_id != '') {
                                $lastPrice = SalesQuatationMeta::selectRaw('rate')->where('item_group_id', $item_id)->where('sales_quatation_id', $request->quotations_id)->first()->rate ?? 0;
                            }
                            $data['itemGroup'][] = [
                                'id' => $item_id,
