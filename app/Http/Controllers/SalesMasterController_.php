...
class SalesMasterController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            return DataTables::of(SalesMaster::with('district', 'taluka', 'village', 'subDivision', 'agentsalesperson')->orderBy('updated_at', 'DESC'))
                ->addIndexColumn()
                ...
                ->addColumn('pdf', function ($row) {
                    if ($row->installation_done == "1") {
                        ...
                        <a class="dropdown-item" href="' . route('self-certification-pdf', $row->id) . '" target="_blank">Self Certification</a>
                        <a class="dropdown-item" href="' . route('request-letter-pdf', $row->id) . '" target="_blank">Request Letter</a>
                        <a class="dropdown-item" href="' . route('model-agreement-pdf', $row->id) . '" target="_blank">Model Agreement</a>
                        <a class="dropdown-item declaration-dcr-link" data-id="' . $row->id . '" data-purchase_order_number="' . $row->purchase_order_number . '" data-purchase_order_date="' . $row->purchase_order_date . '" data-cell_manufacture_name="' . $row->cell_manufacture_name . '" data-cell_gst_invoice_no="' . $row->cell_gst_invoice_no . '" href="#">Declaration DCR</a>
                        <a class="dropdown-item" href="' . route('agreement-pdf', $row->id) . '" target="_blank">Agreement</a>
                        </div>  
                    </div>';
                        return $html;
                    } else {
                ...
                ->addColumn('application_pending', function ($row) use ($user) {
                    ...
                    if ($user->roles[0]->name != 'Manager' && $user->roles[0]->name != 'Sales') {
                        $html .= '<a class="dropdown-item status application-view" href="javascript:void(0);" data-id="' . $row->id . '" data-value="dispach_pending_list">Dispatch Pending List</a>';
                    }

                    $html .= '<a class="dropdown-item status application-view" href="javascript:void(0);" data-id="' . $row->id . '" data-value="meter_installation">Meter Installation</a>
                        <a class="dropdown-item status application-view" href="javascript:void(0);" data-id="' . $row->id . '" data-value="subsidy_claimed">Subsidy Claimed</a>
                        <a class="dropdown-item status application-view" href="javascript:void(0);" data-id="' . $row->id . '" data-value="subsidy_receveid">Subsidy Received</a>';
            ...
            return view('admin.sale.view', compact('agentSalesPerson'));
        }
    }
    ...
    public function statusSave(Request $request)
    {
        try {
            ...
            if (!empty($request->feasibility_discom_sr_number) && !empty($request->feasibility_amount)) {
            ...
            if (!empty($request->installation_asian_person) && $salesMaster->dispach_pending_list == "1") {
    ...
    public function create()
    {
        ...
        return view('admin.sale.add', compact('district', 'agentSalesPerson', 'taluka', 'village', 'subDivision', 'lead_complete', 'sales_quatation'));
    }
    ...
    public function test($file)
    {
        if (!empty($file)) {
            ...
            // Move the uploaded file to the destination directory with the generated filename
    ...
    {
        $salesMaster = SalesMaster::with('district','agentsalesperson', 'taluka', 'document', 'lead', 'installation', 'installation.penalImage', 'installation.invaterImages')->where('id', $id)->first();
        $payment = PaymetCollection::where('sales_master_id', $salesMaster->id)->get();

        if (!is_null($salesMaster)) {
            
            return view('admin.sale.model', compact('salesMaster', 'payment'));
            //$data['html'] = view('admin.sale.model', compact('salesMaster', 'payment'))->render();
    ...
    public function statusView($id, Request $request)
    {
        ...
        $salesMaster = SalesMaster::where('id', $id)->first();
        if (!is_null($salesMaster)) {
            $data['html'] = view('admin.sale.status_model', compact('salesMaster', 'user', 'type'))->render();
            return response()->json($data);
        } else {
            return abort(404);
        }
    ...
    public function edit($id)
    {
        ...
        return view('admin.sale.add', compact('salesMaster', 'agentSalesPerson', 'district', 'taluka', 'village', 'subDivision', 'lead_complete', 'sales_quatation'));
    }
    ...
    public function selfCertificationPdf($id)
    {
        ...
        $pdf = Pdf::loadView('admin.sale.self_certification_pdf', $data);
    ...
    public function requestLetterPdf($id)
    {
        ...
        if ($order_data->sub_division_id != 0) {
            ...
            $pdf = Pdf::loadView('admin.sale.request_letter_pdf', $data);
    ...
    public function modelAgreementPdf($id)
    {
        ...
        $pdf = Pdf::loadView('admin.sale.model_aggriment_pdf', $data);
    ...
    public function declarationDCRPdf($id)
    {
        ...
        $pdf = Pdf::loadView('admin.sale.declaration_dcr_pdf', $data);
    ...
    public function agreementPdf($id)
    {
        ...
        if ($order_data->sub_division_id != 0) {
            ...
            $pdf = Pdf::loadView('admin.sale.agreement_pdf', $data);
