...
class DispachListExport implements FromCollection, WithHeadings, WithMapping
{
    private $request;
    ...
    {
        $query = SalesMaster::select('*', 'id as sid')->with('district','district.state', 'taluka', 'village', 'subDivision', 'salesquatationfull','panel', 'inveter');
        ...
        foreach ($data as $key => $value) {
            ...
            $value->panel_company = (!is_null($value->panel) ? $value->panel->name : 'N/A');
            $value->inverter_company = (!is_null($value->inveter) ? $value->inveter->name : 'N/A');
