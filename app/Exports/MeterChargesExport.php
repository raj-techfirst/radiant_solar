...
class MeterChargesExport implements FromCollection, WithHeadings, WithMapping
{
    private $request;
    ...
    public function collection()
    {
        ...
        if ($this->request->input('payment_ref_no') == "YES") {
            $query->where('payment_ref_number','!=','');
        }
        if ($this->request->input('payment_ref_no') == "NO") {
